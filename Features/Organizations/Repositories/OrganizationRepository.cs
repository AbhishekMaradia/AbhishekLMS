...
namespace LMS_SoulCode.Features.Organizations.Repositories
{
    public class OrganizationRepository : BaseRepository<Organization>, IOrganizationRepository
    {
        ...
        public async Task<(IEnumerable<Organization> Items, int TotalCount)> GetOrganizationsAsync(string? searchTerm, int pageNumber, int pageSize, bool? isActive, int? tenantId, CancellationToken cancellationToken)
        => await GetPagedAsync(
                pageNumber,
                pageSize,
                filter: o => (!isActive.HasValue || o.IsActive == isActive.Value) &&
                            (!tenantId.HasValue || tenantId.Value == 0 || o.Id == tenantId.Value) &&
