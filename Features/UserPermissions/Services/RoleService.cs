...
namespace LMS_SoulCode.Features.UserPermissions.Services
{
    public class RoleService
    {
        ...
        public RoleService(IRoleRepository roleRepository, IMapper mapper, IUserRoleRepository userRoleRepo)
        {
        ...
        public async Task<ApiResponse<List<GetRoleDto>>> UpdateRoleAsync(int id, UpdateRoleDto dto, int? tenantId, CancellationToken cancellationToken = default)
        {
            ...
            // Security: Org Admin can only update within their tenant, cannot change the role's tenant
            if (tenantId.HasValue)
            {
        ...
        public async Task<ApiResponse<List<string>>> DeleteRoleAsync(int id, int? tenantId, CancellationToken cancellationToken = default)
        {
            ...
            // Strict Validation: Cannot delete if users are assigned to this role
            if (await _userRoleRepo.AnyWithRoleAsync(id, cancellationToken))
            {
                return ApiResponse<List<string>>.Fail(Messages.DeleteBlockedRole, StatusCodes.BadRequest);
            }
