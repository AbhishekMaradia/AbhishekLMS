...
namespace LMS_SoulCode.Features.UserPermissions.Services
{
    public class PermissionService
    {
        ...
        public async Task<ApiResponse<string>> DeletePermissionAsync(int id, CancellationToken cancellationToken = default)
        {
            var permission = await _permissionRepo.GetByIdAsync(id, cancellationToken);
            if (permission == null) 
                return ApiResponse<string>.Fail(Messages.NotFound, StatusCodes.NotFound);

            await _permissionRepo.DeleteAsync(id, cancellationToken);
