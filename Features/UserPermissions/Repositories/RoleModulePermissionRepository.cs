    ...
    public class RoleModulePermissionRepository : BaseRepository<RoleModulePermission>, IRoleModulePermissionRepository
    {
            ...
            CancellationToken cancellationToken)
        {
            return await GetPagedAsync<RoleModulePermissionDto>(
                pageNumber,
                pageSize,
                filter: rmp => (!tenantId.HasValue || tenantId.Value == 0 || rmp.TenantId == tenantId.Value) &&
                ...
                projection: q => q.AsNoTracking().OrderByDescending(rmp => rmp.Id).Select(rmp => new RoleModulePermissionDto
                {
                    ...
                    TenantId = rmp.TenantId,
                    OrgName = _context.Organizations.Where(o => o.Id == rmp.TenantId).Select(o => o.Name).FirstOrDefault() ?? "Super Admin",
                    RoleId = rmp.RoleModule.Role.Id,
                    ModuleId = rmp.RoleModule.Module.Id
                }),
                cancellationToken: cancellationToken
            );
        }
