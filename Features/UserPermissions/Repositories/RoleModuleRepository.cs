...
namespace LMS_SoulCode.Features.UserPermissions.Repositories
{
    public class RoleModuleRepository : BaseRepository<RoleModule>, IRoleModuleRepository
    {
        ...
        public async Task<List<RoleModule>> GetByRoleIdAsync(int roleId, int? tenantId, CancellationToken cancellationToken = default)
        {
            ...
            // Apply tenant filtering (SuperAdmin can see all if null)
            if (tenantId.HasValue)
            {
                // This covers records matching the tenant OR global records (IsDefault etc.)
                // Since the query filter handles the basic "matching CurrentTenantId",
                // here we just need to ensure we don't return other tenants' records.
                query = query.Where(rm => rm.TenantId == tenantId.Value || rm.TenantId == null);
            }
            ...
            CancellationToken cancellationToken)
        {
            return await GetPagedAsync<RoleModuleListDto>(
                ...
                filter: rm => rm.Role.IsActive && (rm.Module.IsActive ?? true) &&
                             (!tenantId.HasValue || rm.TenantId == tenantId.Value || rm.TenantId == null) &&
