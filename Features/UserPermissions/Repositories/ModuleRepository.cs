    ...
    public class ModuleRepository : BaseRepository<Module>, IModuleRepository
    {
            ...
            CancellationToken cancellationToken)
        {
            return await GetPagedAsync<GetModuleDto>(
                pageNumber,
                pageSize,
                filter: m => (!isActive.HasValue || m.IsActive == isActive.Value) &&
        ...
        public async Task LinkPermissionsAsync(int moduleId, List<int> permissionIds, int? tenantId, CancellationToken cancellationToken = default)
        {
            var existing = await _context.ModulePermissions
                .IgnoreQueryFilters()
                .Where(mp => mp.ModuleId == moduleId && (tenantId == null || mp.TenantId == tenantId))
                .ToListAsync(cancellationToken);
        ...
        public async Task<List<GetPermissionDto>> GetModulePermissionsAsync(int moduleId, CancellationToken cancellationToken = default)
        {
