...
namespace LMS_SoulCode.Features.Course.Repositories
{
    public interface ICourseRepository : IBaseRepository<CourseEntity>
    {
        ...
        Task<(IEnumerable<CourseEntity> Items, int TotalCount)> GetCoursesByUserGroupAsync(int? groupId, string? searchTerm, int pageNumber, int pageSize, int? tenantId, CancellationToken cancellationToken);
