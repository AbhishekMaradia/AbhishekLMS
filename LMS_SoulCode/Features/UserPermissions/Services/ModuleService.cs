...
namespace LMS_SoulCode.Features.UserPermissions.Services
{
    public class ModuleService
    {
        ...
        public async Task<ApiResponse<List<string>>> AssignPermissionsToModuleAsync(AssignModulePermissionsDto dto, CancellationToken cancellationToken = default)
        {
            await _moduleRepo.LinkPermissionsAsync(dto.ModuleId, dto.PermissionIds, cancellationToken);
            return ApiResponse<List<string>>.Success(new List<string>(), Messages.PermissionsLinked);
        }

        public async Task<ApiResponse<IEnumerable<PermissionDtos.GetPermissionDto>>> GetModulePermissionsAsync(int moduleId, CancellationToken cancellationToken = default)
        {
            var permissions = await _moduleRepo.GetModulePermissionsAsync(moduleId, cancellationToken);
            return ApiResponse<IEnumerable<PermissionDtos.GetPermissionDto>>.Success(permissions, Messages.Success);
        }
    }
}
