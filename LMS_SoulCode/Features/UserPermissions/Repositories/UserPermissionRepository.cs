...
namespace LMS_SoulCode.Features.UserPermissions.Repositories
{
    public class UserPermissionRepository : IUserPermissionRepository
    {
        ...
        public async Task AssignPermissionsAsync(AssignPermissionDto dto, CancellationToken cancellationToken = default)
        {
            // 1. Find or create RoleModule
            var roleModule = await _context.RoleModules
                .FirstOrDefaultAsync(rm => rm.RoleId == dto.RoleId && rm.ModuleId == dto.ModuleId, cancellationToken);
        ...
        public async Task<List<UserPermissionDto>> GetUserPermissionsAsync(int userId, CancellationToken cancellationToken = default)
        {
            ...
            var query = from ur in _context.UserRoles.IgnoreQueryFilters()
                        ...
                        where ur.UserId == userId 
                              ...
                              && (r.TenantId == null || r.TenantId == currentTenantId)
                              && (rm.Role.TenantId == null || rm.Role.TenantId == currentTenantId) // Redundant but safe
        ...
        public async Task<bool> CheckUserPermissionAsync(int userId, string moduleCode, string permissionCode, CancellationToken cancellationToken = default)
        {
            var currentTenantId = _context.CurrentTenantId;
            ...
            CancellationToken cancellationToken)
        {
            ...
            {
                query = query.Where(ur => ur.User.TenantId == tenantId.Value);
            }
        ...
        public async Task<List<UserPermissionDto>> GetRoleModulePermissionsAsync(int roleId, int moduleId, CancellationToken cancellationToken = default)
